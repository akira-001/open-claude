...
Step 4: `.claude/skills/` のスキルファイルを読み込み、関連スキルをコンテキストに注入
         → ユーザーの発言・タスク内容からキーワードを抽出
         → 各スキルファイルの「## トリガー」セクションと照合
         → マッチしたスキルの内容を作業コンテキストとして保持する
         → マッチしなくても、汎用スキル（memory-recall.md）は常にロードする
         → スキルに記載された手順・パターンに従って作業を進めること
         ※ スキルは「参照するだけ」ではなく「従って行動する」こと

Step 5: `cogmem search` で現在の会話コンテキストからキーワード検索
         → score >= 0.75 かつ arousal >= 0.6 のエントリをフラッシュバックとして提示

Step 6: `cogmem signals` で結晶化シグナルをチェック
         → 条件を満たす場合のみ通知を追加

Step 7: トークン予算チェック（目標: 合計 6k tokens）
         → 超過時は /compact を推奨
...
3. `cogmem skills learn` で学習ループを実行:
   ...
   ```
4. スキルの手順に改善が必要な場合、`.claude/skills/` のファイルを直接更新する
...
### 新スキルの自動生成
同じ種類のタスクを3回以上繰り返した場合、パターンを抽出して新しいスキルファイルを `.claude/skills/` に作成する。
