...
3. `cogmem skills learn` で学習ループを実行:
   ...
   ```
4. スキルに改善・新規作成があった場合、`cogmem skills export --force` で `.claude/skills/` に反映:
   ```bash
   cd /Users/akira/workspace/open-claude && cogmem skills export --force
   ```
...
### 新スキルの自動生成
同じ種類のタスクを3回以上繰り返した場合:
1. `cogmem skills create` でスキルをDBに登録
2. `cogmem skills export --force` で `.claude/skills/` に MD ファイルとして出力

※ `.claude/skills/` を直接編集しない。常に cogmem DB → export の流れで統一する。
